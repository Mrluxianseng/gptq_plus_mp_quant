...
def capture_layer0_inputs(
    ...
    *,
    ...
    ``gpu_resident=True`` is the sparse MoE contract: the complete token shard
    is uploaded once, and pre-block/layer-0 modules are asserted CUDA and never
    released to CPU. ``layer_manager`` is consulted for the cpu_master path:
    rank>0's model
    ...
    if gpu_resident and use_manager:
        raise RuntimeError(
            "GPU-resident MoE capture rejects CpuMasterLayerManager."
        )
    if use_manager:
        layer_manager.materialize_runtime_modules(pre_block)
        layers[0] = layer_manager.materialize_layer(0)
    elif gpu_resident:
        _assert_module_cuda(model, dev, label="analyzer.model")
    else:
    ...
    if gpu_resident:
        if torch.is_tensor(samples):
            sample_tensor = samples.reshape(nsamples, -1)
        else:
            sample_tensor = torch.stack(
                [sample.reshape(-1) for sample in samples],
                dim=0,
            )
        # One rank-local H2D upload before capture; the per-sample loop below
        # slices this resident tensor and never calls .to(dev).
        sample_tensor = sample_tensor.to(device=dev, dtype=torch.long)
    ...
    sample_iter = sample_tensor if gpu_resident else samples
    for s in tqdm(
        sample_iter,
        ncols=80,
        desc="Capture layer-0 inputs",
        leave=False,
    ):
        try:
            if gpu_resident:
    ...
    if use_manager:
        ...
        # The full sparse model remains resident from Stage-0 through eval.
        _assert_module_cuda(model, dev, label="analyzer.model")
    else:
