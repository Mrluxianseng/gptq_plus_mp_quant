...
- FULL_BLOCK_REFRESH：是否把每次 Block-GD 更新扩展到当前（以及滑窗激活时下一）Transformer block 中所有尚未量化的线性层。默认 `false`，只更新当前线性层的未量化后缀；REAL-Q Plus 的全 Transformer-block 更新需显式设为 `true`。
