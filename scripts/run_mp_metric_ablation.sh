#!/bin/bash
# Ablation: saliency metric comparison for mixed-precision bit allocation
# Compares fisher_mean vs fisher_max vs fisher_topk vs random at 3.5-bit target
# All experiments share the same static saliency cache (precomputed once).
#
# Usage: ROOT_DIR=/path/to/gptq_plus bash scripts/run_mp_metric_ablation.sh
...
# Shared static cache so all metric runs reuse the same precomputed saliency/fisher
STATIC_CACHE="${ROOT_DIR}/cache/saliency_mp_metric_ablation"
...
echo "================================================"
echo "[1/4] Metric: fisher_mean (default, baseline)"
echo "================================================"
${TORCHRUN} --nnodes=1 --nproc_per_node=1 --rdzv_endpoint=localhost:29400 ptq.py \
    ${COMMON_ARGS} \
    --exp mp_metric_fisher_mean \
    --mp_saliency_metric fisher_mean \
    --w_bits 4

echo "================================================"
echo "[2/4] Metric: fisher_max"
echo "================================================"
${TORCHRUN} --nnodes=1 --nproc_per_node=1 --rdzv_endpoint=localhost:29401 ptq.py \
    ${COMMON_ARGS} \
    --exp mp_metric_fisher_max \
    --mp_saliency_metric fisher_max \
    --w_bits 4

echo "================================================"
echo "[3/4] Metric: fisher_topk (top 10% tokens)"
echo "================================================"
${TORCHRUN} --nnodes=1 --nproc_per_node=1 --rdzv_endpoint=localhost:29402 ptq.py \
    ${COMMON_ARGS} \
    --exp mp_metric_fisher_topk \
    --mp_saliency_metric fisher_topk \
    --mp_topk_ratio 0.1 \
    --w_bits 4

echo "================================================"
echo "[4/4] Metric: random (ablation: no saliency)"
echo "================================================"
${TORCHRUN} --nnodes=1 --nproc_per_node=1 --rdzv_endpoint=localhost:29403 ptq.py \
    ${COMMON_ARGS} \
    --exp mp_metric_random \
    --mp_saliency_metric random \
    --w_bits 4

echo "================================================"
echo "All metric ablations done."
