...
run_fisher_sweep() {
    local model_name="$1"
    local grad_lrs="$2"
    local loss_slide_window="$3"
    local bsz="$4"
    local final_layer_stats_bsz="$5"
    local hessian_accum_bsz="$6"
    local backward_bsz="$7"
    local final_layer_backward_bsz="$8"
    local lm_eval_batch_size="$9"
    local enable_gptq_plus="${10}"
    local base_exp="${11}"
    local global_loss_bsz="${12}"
    local final_layer_grad_lr="${13}"
...
run_step "Qwen3-0.6B fisher_diag_mse grad_lr=2e-4 final_layer_lr=5e-7" \
    run_fisher_sweep \
    "Qwen3-0.6B" \
    "0.0002" \
    "0" \
    ...
    "32" \
    "0" \
    "qwen3_0p6b_fisher_mse_fllr5e7" \
    "1" \
    "0.0000005"

run_step "Qwen3-0.6B fisher_diag_mse grad_lr=2e-4 final_layer_lr=1e-6" \
    run_fisher_sweep \
    "Qwen3-0.6B" \
    "0.0002" \
    "0" \
    ...
    "32" \
    "0" \
    "qwen3_0p6b_fisher_mse_fllr1e6" \
    "1" \
    "0.000001"

run_step "Qwen3-0.6B fisher_diag_mse grad_lr=2e-4 final_layer_lr=2e-6" \
    run_fisher_sweep \
    "Qwen3-0.6B" \
    "0.0002" \
    "0" \
