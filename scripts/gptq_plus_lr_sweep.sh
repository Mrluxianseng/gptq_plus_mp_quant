...
if [[ -z "${A_CLIP_RATIO}" ]]; then
    A_CLIP_RATIO=$([[ "${A_BITS}" -lt 16 ]] && echo 0.9 || echo 1.0)
fi
...
if [[ -z "${K_CLIP_RATIO}" ]]; then
    K_CLIP_RATIO=$([[ "${K_BITS}" -lt 16 ]] && echo 0.9 || echo 1.0)
fi
V_BITS=${V_BITS:-16}
V_GROUPSIZE=${V_GROUPSIZE:--1}
V_ASYM=${V_ASYM:-0}
V_CLIP_RATIO=${V_CLIP_RATIO:-}
if [[ -z "${V_CLIP_RATIO}" ]]; then
    V_CLIP_RATIO=$([[ "${V_BITS}" -lt 16 ]] && echo 0.9 || echo 1.0)
fi
