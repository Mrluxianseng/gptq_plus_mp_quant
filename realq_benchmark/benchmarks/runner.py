    ...
    for summary in summaries:
        if summary["task"] == "humaneval_plus":
            lines.extend(
                [
                    "HumanEval+:",
                    "  python -m realq_benchmark.benchmarks.official_eval \\",
                    "    --task humaneval_plus \\",
...
def run_reasoning_eval(model: Any, tokenizer: Any, cfg: Any) -> dict[str, Any]:
    ...
    try:
        for task in tasks:
            ...
            logging.info(
                "[realq_benchmark] %s: %d examples, %d generations",
                task,
                len(examples),
                len(requests),
            )
    ...
    manifest["elapsed_seconds"] = time.time() - started
    _atomic_json(manifest_path, manifest)
    logging.info("[realq_benchmark] reasoning results saved to %s", root)
    return manifest
