...
class Config:
    ...
    # ----- generation-based reasoning evaluation ---------------------------
    # Appended so every positional field inherited from ``realq.Config`` keeps
    # its original position in the copied package.
    ...
    # This implementation is a fixed zero-shot comparative protocol for
    # quantization studies, not a claim of reproducing every leaderboard's
    # model-specific few-shot prompt.
    ...
    # LiveCodeBench is continuously updated. Never use release_latest for a
    # paper result; pinning v6 makes the evaluated problem set reproducible.
    reasoning_lcb_release: str = "release_v6"
    # The pinned source checkout is used only by the explicit, sandboxed
    # official evaluator. Upstream's current wheel omits evaluator modules.
    reasoning_lcb_source_dir: str = (
        "./datasets/reasoning_eval/vendor/LiveCodeBench"
    )
    # Allows reasoning/lm-eval to run without first building WikiText KL/PPL
    # reference logits. ``skip_eval`` still disables every evaluator.
    skip_kl_ppl_eval: bool = False
    # Exact calibration artifact selected by a controlled experiment.  Keep
    # this benchmark-only addition last so all existing positional Config
    ...
    def __post_init__(self) -> None:
        ...
        if self.reasoning_limit == 0 or self.reasoning_limit < -1:
            raise ValueError("`reasoning_limit` must be -1 or a positive integer.")
