...
from realq_benchmark.benchmarks import data, generation, runner, scoring
from realq_benchmark.config import Config, parse_cli
...
def test_official_eval_refuses_unsandboxed_execution(tmp_path):
    ...
    samples.write_text("{}\n")
    from realq_benchmark.benchmarks import official_eval
...
def test_official_lcb_command_pins_release_and_local_data(monkeypatch, tmp_path):
    ...
    assert "realq_benchmark.benchmarks.lcb_local_eval" in command
