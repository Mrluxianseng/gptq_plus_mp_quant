...
from realq_benchmark import akv, fsdp as realq_fsdp, precompute, runner
from realq_benchmark.benchmarks import run_reasoning_eval
...
def _run_generation_evals_if_requested(
    cfg: "Config",
    analyzer: model_utils.ModelAnalyzer,
) -> bool:
    """Run rank-0 QA/reasoning eval after releasing the process group.

    Returns ``True`` when either evaluator was requested and the caller must return
    ...
    the now-available visible GPUs and drives generation.
    """
    requested = (
...
def run(cfg: "Config") -> None:
    ...
    if loaded_checkpoint is not None:
        ...
        if _run_generation_evals_if_requested(cfg, analyzer):
            return
        if parallel_env.is_dist_available_and_initialized():
            parallel_env.barrier()
        return
