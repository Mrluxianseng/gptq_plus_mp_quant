#!/usr/bin/env python3
"""V2 LR analysis/freeze gate bound to the paired-cache campaign."""
...
SELECTION_ID = "realq-fullmodel-two-branch-lr-selection-20260817-v2-paired-cache"
SELECTION_PATH = v2.OUTPUT_ROOT / "selections.json"
core.SELECTION_ID = SELECTION_ID
core.SELECTION_PATH = SELECTION_PATH


def _zero_audit_status() -> dict[str, Any]:
    if not v2.ZERO_AUDIT_PATH.is_file():
        return {"status": "pending", "path": str(v2.ZERO_AUDIT_PATH)}
    value = c._read_json(v2.ZERO_AUDIT_PATH)
    return {
        "status": value.get("status"),
        "path": str(v2.ZERO_AUDIT_PATH),
        "audit_fingerprint": value.get("audit_fingerprint"),
...
def _status(args: argparse.Namespace) -> int:
    ...
    payload["paired_zero_control"] = _zero_audit_status()
    if args.output:
...
def _freeze(_: argparse.Namespace) -> int:
    zero_audit = v2._load_zero_audit()
    ...
    body = {
        ...
        "campaign_id": v2.CAMPAIGN_ID,
        "protocol_fingerprint": plan["protocol_fingerprint"],
        "plan": {"path": str(v2.PLAN_PATH), "sha256": c._file_sha256(v2.PLAN_PATH)},
        "paired_zero_control_audit": {
            "path": str(v2.ZERO_AUDIT_PATH),
            "sha256": c._file_sha256(v2.ZERO_AUDIT_PATH),
            "audit_fingerprint": zero_audit["audit_fingerprint"],
        },
        "selection_code": [
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"realq-fullmodel-selection-v2: {exc}", file=sys.stderr)
        return 2
