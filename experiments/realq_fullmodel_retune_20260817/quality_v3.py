#!/usr/bin/env python3
"""Checkpoint-only WikiText2 PPL/KL and ten-task QA stage for V3."""
...
QUALITY_ID = "realq-fullmodel-two-branch-quality-20260817-v3"
QUALITY_PLAN_PATH = v3.OUTPUT_ROOT / "quality_plan.json"
QUALITY_AUDIT_PATH = v3.OUTPUT_ROOT / "quality_final_audit.json"
...
def _quality_dir(branch: str, config: str) -> Path:
    return v3.OUTPUT_ROOT / "quality" / branch / config
...
def _formal_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    # Re-audit every marker/checkpoint rather than trusting only the aggregate.
...
def _quality_command(
    ...
    c._set_arg(command, "--exp", "v3_wikitext2_paperqa")
...
def _load_plan() -> dict[str, Any]:
    ...
    fingerprint = value.get("quality_plan_fingerprint")
    identity = dict(value)
    identity.pop("quality_plan_fingerprint", None)
    if c._canonical_sha256(identity) != fingerprint:
        ...
        rows.append(
            {
                "branch": branch,
                "config": config,
                "status": "succeeded" if (directory / "quality_success.json").is_file() else "running" if (directory / CLAIM_NAME).is_dir() else "pending",
            }
        )
    print(
        json.dumps(
...
def _parser() -> argparse.ArgumentParser:
    ...
    manifest.add_argument("--name", default="quality_v3_v1")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"realq-quality-v3: {exc}", file=sys.stderr)
        return 2
