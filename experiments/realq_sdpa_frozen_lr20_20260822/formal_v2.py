#!/usr/bin/env python3
"""Formal checkpoints for deterministic-SDPA V2 memory fallback."""
...
from experiments.realq_sdpa_frozen_lr20_20260822 import formal as formal_v1
...
FORMAL_ID = "realq-sdpa-frozen-run15-lr20-formal-20260822-v2b-memory"
FORMAL_PLAN_PATH = campaign.OUTPUT_ROOT / "formal_plan_v2b.json"
FORMAL_AUDIT_PATH = campaign.OUTPUT_ROOT / "formal_final_audit_v2b.json"
_IMPLEMENTATION_PATH = Path(core.__file__).resolve()
# Importing the dynamic controller first also imports the V1 formal adapter,
# which deliberately rebinds ``core._build_plan``.  Capture the pristine V3
# lifecycle from that adapter's immutable reference, not from mutable module
# globals, so plan construction is import-order invariant.
_BASE_BUILD_PLAN = formal_v1._BASE_BUILD_PLAN
...
def _flags(command: Sequence[str]) -> dict[str, str]:
    return campaign.v1._flags(command)


def _worker_env(gpu: str) -> dict[str, str]:
    _restore_campaign_bindings()
    return campaign.v1._worker_environment(gpu)
...
def _formal_command(
    ...
    c._set_arg(command, "--exp", "formal_sdpa_frozen_run15_lr20_v2_memory")
...
def _build_plan() -> dict[str, Any]:
    ...
    body["protocol"] = {
        **body["protocol"],
        ...
        "global_loss_bsz_by_model": dict(campaign.GLOBAL_LOSS_BSZ),
        "capacity_fallback_scope": "only V1 OOM models; one documented ladder rung",
        "fisher_hessian_tf32_is_scoped": True,
        "qwen3_32b_hessian_accum_bsz": 32,
...
def _activate() -> None:
    _restore_campaign_bindings()
    # The reusable V3 lifecycle reaches the balanced scheduler through
    # ``v3.v2``.  V2-memory deliberately wraps the SDPA campaign instead of
    # duplicating that scheduler module, so expose the inherited binding at
    # runtime without altering the frozen campaign source.
    campaign.v2 = campaign.v1.v2
    core.v3 = campaign
