#!/usr/bin/env python3
"""Eager quality wave for the third audited Q32 SDPA checkpoint.
...
row executor in a fresh process, freezes exactly one newly completed Q32 row, and
...
EAGER_ID = "realq-sdpa-v5-merged-eager-quality-q32-wave2-20260823-v1"
OUTPUT_ROOT = campaign.OUTPUT_ROOT / "eager_quality_q32_wave2_v1"
...
def _pairs() -> list[tuple[str, str]]:
    ...
    if len(pairs) != EXPECTED_ROW_COUNT or len(set(pairs)) != EXPECTED_ROW_COUNT:
        raise core.EagerQualityError("Q32 wave2 must contain exactly one row")
    return pairs
...
def _audit(args: argparse.Namespace) -> int:
    ...
    if args.require_complete and len(rows) != EXPECTED_ROW_COUNT:
        raise core.EagerQualityError(
            f"Q32 wave2 audit incomplete: {len(rows)}/{EXPECTED_ROW_COUNT}"
        )
    value: dict[str, Any] = {
        "schema_version": 1,
...
def _adopt(args: argparse.Namespace) -> int:
    ...
    for branch, config in _pairs():
        ...
        if (
            ...
            raise core.EagerQualityError(
                f"canonical Q32 wave2 adoption mismatch: {branch}/{config}"
            )
    ...
    if len(candidates) != EXPECTED_ROW_COUNT:
        raise core.EagerQualityError("Q32 wave2 adoption preflight row mismatch")
    ...
    for branch, config, eager_identity, eager_result, official_row, directory in candidates:
        ...
        adoption = {
            "kind": "eager_q32_wave2_quality_adoption",
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "status": "complete",
        "kind": "eager_q32_wave2_quality_canonical_adoption",
        "eager_id": EAGER_ID,
        "quality_id": quality.QUALITY_ID,
        "quality_plan_fingerprint": official_plan["quality_plan_fingerprint"],
