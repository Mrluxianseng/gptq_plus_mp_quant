#!/usr/bin/env python3
"""Merge 32 V2b checkpoints with eight Q32 Hessian-accum-16 recoveries."""
...
from experiments.realq_sdpa_frozen_lr20_20260822 import formal_v3_q32_haccum16 as q32


FORMAL_ID = "realq-sdpa-frozen-run15-lr20-formal-20260822-v3-merged"
MERGE_ID = FORMAL_ID
FORMAL_PLAN_PATH = campaign.OUTPUT_ROOT / "formal_plan_v3_merged.json"
FORMAL_AUDIT_PATH = campaign.OUTPUT_ROOT / "formal_final_audit_v3_merged.json"
...
def _build_plan() -> dict[str, Any]:
    ...
    source_rows = {
        ...
        "v3_q32_haccum16": _plan_rows(q32_plan),
    }
    rows = []
    for branch, config in _balanced_pairs():
        source = "v3_q32_haccum16" if _is_q32(config) else "v2b_non_q32"
        key = f"{branch}/{config}"
        ...
        expected_haccum = "16" if _is_q32(config) else "64"
        if (
            flags.get("--attention_backend") != "sdpa"
            or flags.get("--hessian_accum_bsz") != expected_haccum
    ...
    body: dict[str, Any] = {
        ...
        "protocol": {
            ...
            "q32_source": "V3 deterministic-SDPA recovery",
            "q32_only_delta": "hessian_accum_bsz 32 -> 16",
