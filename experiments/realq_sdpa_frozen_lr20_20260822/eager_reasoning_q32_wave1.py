#!/usr/bin/env python3
"""Eager reasoning wave for the first two audited Q32 SDPA checkpoints.

The immutable 93-work non-Q32 runner remains byte-stable.  This module
configures its audited row executor in a fresh process for six Q32 generation
...
EAGER_ID = "realq-sdpa-v5-merged-eager-reasoning-q32-wave1-20260823-v1"
OUTPUT_ROOT = campaign.OUTPUT_ROOT / "eager_reasoning_q32_wave1_v1"
...
def _pairs() -> list[tuple[str, str]]:
    ...
    if len(pairs) != EXPECTED_PAIR_COUNT or len(set(pairs)) != EXPECTED_PAIR_COUNT:
        raise core.EagerReasoningError("Q32 reasoning wave1 pair matrix drifted")
    return pairs
...
def _works() -> list[tuple[str, str, str]]:
    ...
    if len(works) != EXPECTED_WORK_COUNT or len(set(works)) != EXPECTED_WORK_COUNT:
        raise core.EagerReasoningError("Q32 reasoning wave1 work matrix drifted")
    return works
...
def _audit(args: argparse.Namespace) -> int:
    ...
    if args.require_complete and len(rows) != EXPECTED_WORK_COUNT:
        raise core.EagerReasoningError(
            f"Q32 reasoning wave1 audit incomplete: {len(rows)}/{EXPECTED_WORK_COUNT}"
        )
    value: dict[str, Any] = {
        "schema_version": 1,
...
def _adopt(args: argparse.Namespace) -> int:
    ...
    for branch, config, task in _works():
        ...
        if (
            ...
            raise core.EagerReasoningError(
                f"canonical Q32 wave1 adoption mismatch: {work_id}"
            )
    ...
    if len(candidates) != EXPECTED_WORK_COUNT:
        raise core.EagerReasoningError("Q32 reasoning wave1 adoption row mismatch")
    ...
    for branch, config, task, eager_identity, eager_result, official_row, output in candidates:
        ...
        attempt.mkdir()
        adoption = {
            "kind": "eager_q32_wave1_reasoning_adoption",
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "status": "complete",
        "kind": "eager_q32_wave1_reasoning_canonical_adoption",
        "eager_id": EAGER_ID,
        "reasoning_id": reasoning.REASONING_ID,
        "reasoning_plan_fingerprint": official_plan["reasoning_plan_fingerprint"],
