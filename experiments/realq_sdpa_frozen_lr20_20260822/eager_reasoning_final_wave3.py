#!/usr/bin/env python3
"""Eager reasoning wave for two terminal checkpoints outside earlier waves.

The immutable 93-work non-Q32 runner remains byte-stable.  This module
configures its audited row executor for REALQ-F Llama-W4 and Q32-W4, producing
six generation works for later adoption against the canonical 120-work plan.
"""
...
EAGER_ID = "realq-sdpa-v5-merged-eager-reasoning-final-wave3-20260823-v1"
OUTPUT_ROOT = campaign.OUTPUT_ROOT / "eager_reasoning_final_wave3_v1"
...
def _pairs() -> list[tuple[str, str]]:
    ...
    if len(pairs) != EXPECTED_PAIR_COUNT or len(set(pairs)) != EXPECTED_PAIR_COUNT:
        raise core.EagerReasoningError("final reasoning wave3 pair matrix drifted")
    return pairs
...
def _works() -> list[tuple[str, str, str]]:
    ...
    if len(works) != EXPECTED_WORK_COUNT or len(set(works)) != EXPECTED_WORK_COUNT:
        raise core.EagerReasoningError("final reasoning wave3 work matrix drifted")
    return works
...
def _build_plan() -> dict[str, Any]:
    ...
    body["protocol"] = {
        **body["protocol"],
        ...
        "scope": "terminal REALQ-F Llama-W4 and Q32-W4 rows",
...
def _run_worker(args: argparse.Namespace) -> int:
    ...
    if not hostname.startswith("j-") or not hostname.endswith("-master-0"):
        raise core.EagerReasoningError("final eager worker must run in a debug pod")
...
def _audit(args: argparse.Namespace) -> int:
    ...
    if args.require_complete and len(rows) != EXPECTED_WORK_COUNT:
        raise core.EagerReasoningError(
            f"final reasoning wave3 audit incomplete: {len(rows)}/{EXPECTED_WORK_COUNT}"
        )
    value: dict[str, Any] = {
        "schema_version": 1,
...
def _adopt(args: argparse.Namespace) -> int:
    ...
    for branch, config, task in _works():
        ...
        if (
            ...
            raise core.EagerReasoningError(
                f"canonical final wave3 adoption mismatch: {work_id}"
            )
    ...
    if len(candidates) != EXPECTED_WORK_COUNT:
        raise core.EagerReasoningError("final reasoning wave3 adoption row mismatch")
    ...
    for branch, config, task, eager_identity, eager_result, official_row, output in candidates:
        ...
        attempt.mkdir()
        adoption = {
            "kind": "eager_final_wave3_reasoning_adoption",
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "status": "complete",
        "kind": "eager_final_wave3_reasoning_canonical_adoption",
        "eager_id": EAGER_ID,
        "reasoning_id": reasoning.REASONING_ID,
        "reasoning_plan_fingerprint": official_plan["reasoning_plan_fingerprint"],
