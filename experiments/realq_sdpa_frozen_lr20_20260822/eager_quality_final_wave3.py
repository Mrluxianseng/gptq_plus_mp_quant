#!/usr/bin/env python3
"""Eager quality wave for two terminal SDPA checkpoints outside earlier waves.
...
row executor in a fresh process, freezes the terminal REALQ-F Llama-W4 and
Q32-W4 rows, and keeps evidence isolated until the canonical plan exists.
"""
...
EAGER_ID = "realq-sdpa-v5-merged-eager-quality-final-wave3-20260823-v1"
OUTPUT_ROOT = campaign.OUTPUT_ROOT / "eager_quality_final_wave3_v1"
...
def _pairs() -> list[tuple[str, str]]:
    ...
    if len(pairs) != EXPECTED_ROW_COUNT or len(set(pairs)) != EXPECTED_ROW_COUNT:
        raise core.EagerQualityError("final wave3 must contain exactly two rows")
    return pairs
...
def _run_worker(args: argparse.Namespace) -> int:
    ...
    if not hostname.startswith("j-") or not hostname.endswith("-master-0"):
        raise core.EagerQualityError("final eager worker must run in a debug pod")
...
def _audit(args: argparse.Namespace) -> int:
    ...
    if args.require_complete and len(rows) != EXPECTED_ROW_COUNT:
        raise core.EagerQualityError(
            f"final wave3 audit incomplete: {len(rows)}/{EXPECTED_ROW_COUNT}"
        )
    value: dict[str, Any] = {
        "schema_version": 1,
...
def _adopt(args: argparse.Namespace) -> int:
    ...
    for branch, config in _pairs():
        ...
        if (
            ...
            raise core.EagerQualityError(
                f"canonical final wave3 adoption mismatch: {branch}/{config}"
            )
    ...
    if len(candidates) != EXPECTED_ROW_COUNT:
        raise core.EagerQualityError("final wave3 adoption preflight row mismatch")
    ...
    for branch, config, eager_identity, eager_result, official_row, directory in candidates:
        ...
        adoption = {
            "kind": "eager_final_wave3_quality_adoption",
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "status": "complete",
        "kind": "eager_final_wave3_quality_canonical_adoption",
        "eager_id": EAGER_ID,
        "quality_id": quality.QUALITY_ID,
        "quality_plan_fingerprint": official_plan["quality_plan_fingerprint"],
