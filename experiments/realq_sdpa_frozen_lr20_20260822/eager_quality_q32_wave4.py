#!/usr/bin/env python3
"""Eager quality wave for the four remaining Q32 SDPA checkpoints.
...
row executor in a fresh process, freezes exactly the four Q32 rows outside
earlier waves, and keeps all evidence isolated until the canonical forty-row
quality plan exists.
"""
...
EAGER_ID = "realq-sdpa-v5-merged-eager-quality-q32-wave4-20260823-v1"
OUTPUT_ROOT = campaign.OUTPUT_ROOT / "eager_quality_q32_wave4_v1"
...
def _pairs() -> list[tuple[str, str]]:
    ...
    if len(pairs) != EXPECTED_ROW_COUNT or len(set(pairs)) != EXPECTED_ROW_COUNT:
        raise core.EagerQualityError("Q32 wave4 must contain exactly four rows")
    return pairs
...
def _audit(args: argparse.Namespace) -> int:
    ...
    if args.require_complete and len(rows) != EXPECTED_ROW_COUNT:
        raise core.EagerQualityError(
            f"Q32 wave4 audit incomplete: {len(rows)}/{EXPECTED_ROW_COUNT}"
        )
    value: dict[str, Any] = {
        "schema_version": 1,
...
def _adopt(args: argparse.Namespace) -> int:
    ...
    for branch, config in _pairs():
        ...
        if (
            ...
            raise core.EagerQualityError(
                f"canonical Q32 wave4 adoption mismatch: {branch}/{config}"
            )
    ...
    if len(candidates) != EXPECTED_ROW_COUNT:
        raise core.EagerQualityError("Q32 wave4 adoption preflight row mismatch")
    ...
    for branch, config, eager_identity, eager_result, official_row, directory in candidates:
        ...
        adoption = {
            "kind": "eager_q32_wave4_quality_adoption",
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "status": "complete",
        "kind": "eager_q32_wave4_quality_canonical_adoption",
        "eager_id": EAGER_ID,
        "quality_id": quality.QUALITY_ID,
        "quality_plan_fingerprint": official_plan["quality_plan_fingerprint"],
