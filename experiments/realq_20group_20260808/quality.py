...
QUALITY_ID = "realq-20group-wikitext2-paperqa-20260809-v1"
...
def run_node(root: Path, node: int) -> None:
    ...
    for run in runs:
        ...
        if marker.is_file():
            ...
            pending.append(("quant", run))
    bf16_works = bf16.expected_works_for_node(node)
    require(len(bf16_works) == 10, f"node {node} BF16 assignment must contain ten works")
    for work in bf16_works:
        directory = bf16.work_dir(root, work)
        marker = directory / "bf16_success.json"
        if marker.is_file():
            bf16.audit_one(root, bf16_plan, work)
        elif (directory / bf16.CLAIM_NAME).is_dir():
            owner = c.read_json(directory / bf16.CLAIM_NAME / "owner.json")
            if owner.get("host") == socket.gethostname():
                reserved_gpus.add(int(owner["gpu"]))
            continue
        else:
            pending.append(("bf16", work))
    active: dict[subprocess.Popen[Any], tuple[str, Any, int]] = {}
        ...
        while pending and free_gpus and not failures:
            ...
            if kind == "quant":
                command = [
                    ...
                    "--root", str(root), "--run-id", item.run_id,
                    "--gpu", str(gpu),
                ]
                label = item.run_id
            else:
                command = [
                    str(c.PYTHON), "-m", bf16.MODULE, "_worker",
                    "--root", str(root), "--work-id", item.work_id,
                    "--gpu", str(gpu),
                ]
                label = item.work_id
            ...
            active[process] = (kind, item, gpu)
        if not active:
            break
        time.sleep(2)
        for process, (kind, item, gpu) in list(active.items()):
            ...
            free_gpus.sort()
            if returncode != 0:
                label = item.run_id if kind == "quant" else item.work_id
                failures.append(f"{label}:rc={returncode}")
    ...
    for work in bf16_works:
        marker = bf16.work_dir(root, work) / "bf16_success.json"
        if marker.is_file():
            bf16.audit_one(root, bf16_plan, work)
        else:
            require(
                (bf16.work_dir(root, work) / bf16.CLAIM_NAME).is_dir(),
                f"node {node} BF16 work missing after scheduler: {work.work_id}",
            )
...
def watch(root: Path, poll_seconds: int) -> None:
    ...
    while True:
        ...
        bf16_count = bf16.success_count(root)
        print(
            f"[{c.now()}] quant_quality_success={count}/20 "
            f"bf16_work_success={bf16_count}/20",
            flush=True,
        )
        if count == len(c.RUNS) and bf16_count == len(bf16.WORKS):
            result = audit_all(root)
            bf16_result = bf16.audit_all(root)
            print(
                json.dumps(
                    {"quant": result["counts"], "bf16": bf16_result["counts"]},
                    sort_keys=True,
                ),
                flush=True,
            )
