...
def audit_plan(root: Path) -> dict[str, Any]:
    ...
    require(
        ...
        f"plan is not the frozen linear-refresh identity: {fingerprint}",
    )
    require(
    ...
    require(
        plan.get("full_block_refresh") is False
        and plan.get("weight_update_scope")
        == "current_linear_trailing_columns",
        f"plan refresh-scope mismatch: {plan_path}",
    )
    require(
