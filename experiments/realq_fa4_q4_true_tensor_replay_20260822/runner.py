#!/usr/bin/env python3
"""Immutable single-GPU campaign for Qwen3-4B true attention replay."""
...
SOURCE_FILES = (
    "experiments/realq_fa4_q4_true_tensor_replay_20260822/__init__.py",
    "experiments/realq_fa4_q4_true_tensor_replay_20260822/common.py",
    "experiments/realq_fa4_q4_true_tensor_replay_20260822/capture_driver.py",
    "experiments/realq_fa4_q4_true_tensor_replay_20260822/replay_worker.py",
    "experiments/realq_fa4_q4_true_tensor_replay_20260822/compare_worker.py",
    "experiments/realq_fa4_q4_true_tensor_replay_20260822/runner.py",
...
def _capture_command() -> list[str]:
    ...
    command[2] = (
        "experiments.realq_fa4_q4_true_tensor_replay_20260822.capture_driver"
    )
    ...
    overrides = {
        ...
        "--exp": "qwen3_4b_true_tensor_capture_math_sdpa",
        "--skip_eval": "true",
...
def _worker_command(arm: str) -> list[str]:
    ...
    return [
        source_command[0],
        "-m",
        "experiments.realq_fa4_q4_true_tensor_replay_20260822.replay_worker",
        "--arm",
        arm,
    ]
...
def _comparison_command() -> list[str]:
    ...
    return [
        source_command[0],
        "-m",
        "experiments.realq_fa4_q4_true_tensor_replay_20260822.compare_worker",
    ]
...
def build_plan(physical_gpu: int, *, verify_model_hashes: bool) -> dict[str, Any]:
    ...
    body = {
        "schema_version": 1,
        "experiment_id": "realq-fa4-q4-true-tensor-replay-20260822-v2",
        ...
        "capture": {
            ...
            "dout_boundary": "math_sdpa output pre_reshape_pre_o_proj",
            "loss": "fixed-label summed NLL times production LOSS_GRAD_SCALE=1000",
            "attention_mask": {
                "sdpa_capture": (
                    "unchanged Transformers canonical bool lower-triangular mask; "
                    "shape Bx1x2048x2048"
                ),
                "fa4_production": "attention_mask=None, causal=true",
                "semantic_relation": "exactly equivalent dense causal attention",
            },
        ...
        "resource_contract": {
            ...
            "capture_peak_gpu_memory_note": (
                "full rotated Qwen3-4B math-SDPA first-batch graph; run only on "
...
def _validate_capture(plan: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or receipt.get("attention_backend") != "math_sdpa"
    ...
    mask_tensor = mask_contract.get("tensor", {})
    expected_true = (
        common.BATCH_SIZE
        * common.SEQUENCE_LENGTH
        * (common.SEQUENCE_LENGTH + 1)
        // 2
    )
    if (
        mask.get("calls") != 36
        or mask.get("passed_to_math_sdpa_unchanged") is not True
        or mask_contract.get("representation")
        != "canonical_bool_lower_triangular_inclusive"
        or mask_contract.get("true_values") != expected_true
        or mask_tensor.get("shape")
        != [common.BATCH_SIZE, 1, common.SEQUENCE_LENGTH, common.SEQUENCE_LENGTH]
        or mask_tensor.get("dtype") != "torch.bool"
        or not mask_tensor.get("semantic_sha256")
        or not mask_tensor.get("layout_semantic_sha256")
    ):
...
def pipeline() -> dict[str, Any]:
    ...
    final = {
        "schema_version": 1,
        "status": "completed",
        "kind": "qwen3_4b_true_attention_replay_campaign",
...
def launch() -> dict[str, Any]:
    ...
    command = [
        ...
        "-u",
        "-m",
        "experiments.realq_fa4_q4_true_tensor_replay_20260822.runner",
        "--pipeline",
    ]
