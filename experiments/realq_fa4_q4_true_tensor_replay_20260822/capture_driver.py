#!/usr/bin/env python3
"""Capture Qwen3-4B's true post-RoPE Q/K/V and matching upstream dO.
...
class CaptureController:
    ...
    def _validate_attention_mask(
        self, attention_mask: torch.Tensor | None, query: torch.Tensor
    ) -> None:
        """Bind the canonical boolean mask used by the production SDPA path."""

        if not torch.is_tensor(attention_mask):
            raise common.ReplayDiagnosticError(
                "true-tensor SDPA capture requires its canonical boolean mask"
            )
        expected_shape = (
            common.BATCH_SIZE,
            1,
        ...
        if (
            tuple(attention_mask.shape) != expected_shape
            or attention_mask.dtype != torch.bool
            or attention_mask.device != query.device
        ):
            raise common.ReplayDiagnosticError(
                "SDPA attention-mask contract changed: "
                f"{common.tensor_layout(attention_mask)}"
            )
        if self._attention_mask is None:
            positions = torch.arange(
                common.SEQUENCE_LENGTH, device=attention_mask.device
            )
            canonical = (positions[:, None] >= positions[None, :]).view(
                1, 1, common.SEQUENCE_LENGTH, common.SEQUENCE_LENGTH
            )
            canonical = canonical.expand(common.BATCH_SIZE, -1, -1, -1)
            if not torch.equal(attention_mask, canonical):
                raise common.ReplayDiagnosticError(
                    "SDPA attention mask is not dense lower-triangular causal"
                )
            self._attention_mask = attention_mask
            self.attention_mask_contract = {
                "representation": "canonical_bool_lower_triangular_inclusive",
                "tensor": common.tensor_contract(attention_mask),
                "true_values": int(attention_mask.sum().item()),
            }
        elif attention_mask is not self._attention_mask:
            raise common.ReplayDiagnosticError(
                "decoder layers stopped sharing the frozen SDPA causal mask"
            )
    ...
    def _attention_wrapper(self, original: Any):
        def wrapped(
            ...
            **kwargs: Any,
            ...
            self._validate_attention_mask(attention_mask, query)
    ...
    def run(self, cfg: Any, analyzer: Any):
        ...
        if (
            ...
            or str(cfg.attention_backend) != "sdpa"
        ...
        ALL_ATTENTION_FUNCTIONS["sdpa"] = self._attention_wrapper(original_sdpa)
        try:
            ...
            if self.attention_mask_calls != 36 or self.attention_mask_contract is None:
                raise common.ReplayDiagnosticError(
                    "SDPA causal mask was not observed once at every decoder layer"
                )
...
def main() -> int:
    ...
    receipt = {
        ...
        "kind": "realq_qwen3_4b_true_attention_tensor_capture",
        ...
        "attention_mask": {
            ...
            "passed_to_math_sdpa_unchanged": True,
            "semantic_equivalent_fa4_arguments": {
                "attention_mask": None,
                "causal": True,
            },
        },
