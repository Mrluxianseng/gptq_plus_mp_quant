...
def _load_sdpa_trajectory_capture() -> dict[str, Any]:
    path = common.SDPA_TRAJECTORY_CAPTURE_RECEIPT
    if (
        not path.is_file()
        or common.file_sha256(path)
        != common.SDPA_TRAJECTORY_CAPTURE_RECEIPT_SHA256
    ):
        raise common.ReplayDiagnosticError("frozen SDPA trajectory receipt changed")
    receipt = common.read_json(path)
    if (
        receipt.get("status") != "completed"
        or receipt.get("plan_fingerprint")
        != common.SDPA_TRAJECTORY_PLAN_FINGERPRINT
        or receipt.get("selected_layers") != list(common.SELECTED_LAYERS)
        or receipt.get("attention_backend") != "math_sdpa"
        or receipt.get("production_layouts_sha256")
        != common.canonical_sha256(receipt.get("production_layouts", {}))
    ):
        raise common.ReplayDiagnosticError("frozen SDPA trajectory contract changed")
    return receipt
...
def execute(
    ...
    sdpa_trajectory_capture = _load_sdpa_trajectory_capture()
    ...
    for layer in common.SELECTED_LAYERS:
        ...
        sdpa_trajectory = _load_capture_kind(
            sdpa_trajectory_capture, layer, "real"
        )
        trajectory_comparison = {
            "left": "math_sdpa_side",
            "right": "flash_attention_4_side",
            "metrics_scope": "all tensor elements; no sampling",
            "fields": {
                field: common.full_tensor_metrics(
                    sdpa_trajectory[field], sources["real"][field]
                )
                for field in ("q", "k", "v", "dout")
            },
            "samples": {
                str(sample): {
                    "fields": {
                        field: common.full_tensor_metrics(
                            sdpa_trajectory[field][sample : sample + 1],
                            sources["real"][field][sample : sample + 1],
                        )
                        for field in ("q", "k", "v", "dout")
                    }
                }
        ...
        layers[str(layer)] = {
            "source_trajectory_comparison": trajectory_comparison,
            "cases": case_results,
    ...
    receipt = {
        ...
        "sdpa_trajectory_capture_receipt": str(
            common.SDPA_TRAJECTORY_CAPTURE_RECEIPT
        ),
        "sdpa_trajectory_capture_receipt_sha256": (
            common.SDPA_TRAJECTORY_CAPTURE_RECEIPT_SHA256
        ),
        "replay_receipts": {
            arm: {
