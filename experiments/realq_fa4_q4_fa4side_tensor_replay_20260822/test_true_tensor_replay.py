...
from experiments.realq_fa4_q4_fa4side_tensor_replay_20260822 import (
    capture_driver,
    common,
    compare_worker,
    runner,
...
def test_capture_binds_fa4_dense_causal_contract(tmp_path) -> None:
    controller = capture_driver.CaptureController(tmp_path, "test")
    module = torch.nn.Module()
    module.is_causal = True
    controller._validate_fa4_contract(None, module, {"is_causal": True})
    controller._validate_fa4_contract(None, module, {"is_causal": True})
    assert controller.attention_mask_calls == 2
    assert controller.attention_mask_contract is not None
    assert controller.attention_mask_contract["representation"] == "none_dense_unpadded"
    assert controller.attention_mask_contract["effective_is_causal"] is True
    other = capture_driver.CaptureController(tmp_path, "test-bad")
    with pytest.raises(common.ReplayDiagnosticError, match="attention_mask=None"):
        other._validate_fa4_contract(torch.ones(1, dtype=torch.bool), module, {})
    with pytest.raises(common.ReplayDiagnosticError, match="causal attention"):
        other._validate_fa4_contract(None, module, {"is_causal": False})
