#!/usr/bin/env python3
"""Safely hand one completed evaluation lane to the REAL-Q diagnostic.

This helper is deliberately narrow.  The evaluation parent was SIGSTOP'ed
while its child continued normally, preventing it from claiming another
suite.  Once the child has atomically published ``suite_success.json`` and is
no longer a CUDA process, this helper records the otherwise-unreaped attempt,
releases its claim, and terminates only the stopped parent.  Releasing the
physical-GPU lock wakes the already queued REAL-Q supervisor.
"""
...
def execute(*, parent_pid: int, child_pid: int, eval_id: str, gpu: int) -> dict:
    ...
    parent_state = _process_state(parent_pid)
    parent_cmd = _cmdline(parent_pid)
    expected = (
        "experiments.additional_methods_fair20_eval_20260821.worker "
        f"--physical-gpu {gpu}"
    )
    if parent_state != "T" or expected not in parent_cmd:
        raise HandoffError(
            f"parent is not the expected stopped worker: state={parent_state!r}"
        )
    child_state = _process_state(child_pid)
    ...
    while time.monotonic() < deadline and _process_state(parent_pid) not in {None, "Z"}:
        time.sleep(0.05)
    receipt = {
        "schema_version": 1,
        "status": "succeeded",
        ...
        "parent_state_after_sigkill": _process_state(parent_pid),
        "finished_at": common.now(),
    }
    output = runner.OUTPUT_ROOT / "lane_supervisor" / hostname / f"gpu{gpu}"
    output.mkdir(parents=True, exist_ok=True)
    common.atomic_json(output / "handoff_receipt.json", receipt)
    return receipt
